...
| FD I/O: `read`, `write`, `readv`, `writev`, `pread64`, `lseek`, `getdents64`, `sendfile`, `dup`, `dup3`, `fcntl` | `partial` | `os/src/fs/fd.rs`, `os/src/syscall/fs.rs` | Regular files, dirs, console, and pipes are covered. MemFS and ext4 regular fds track read/write access, `O_APPEND`, and fixed-offset `pread64` without moving the fd offset. `write` appends at current EOF when `O_APPEND` is set; `lseek` allows seeking past EOF without changing file size, rejects negative/overflow positions, and returns `ESPIPE` for pipes. Writes and truncate/ftruncate update file size and metadata-backed block counts; large offset/count overflow returns `EFBIG`. Pipes now use bounded in-kernel buffering with blocking read/write through wait queues and `O_NONBLOCK` returning `EAGAIN` when the operation would sleep. Per-fd `FD_CLOEXEC` is tracked for `openat(O_CLOEXEC)`, `pipe2(O_CLOEXEC)`, `dup3(O_CLOEXEC)`, `fcntl(F_GETFD/F_SETFD/F_DUPFD_CLOEXEC)`, and `execve` close. `fcntl(F_SETFL)` mutates pipe `O_NONBLOCK` and regular-file `O_APPEND`; descriptor-shared offsets/status flags and `pwrite*`/`preadv*` remain `partial`/`missing`. |
| Pipes and polling: `pipe2`, `ppoll`, `pselect6` | `partial` | `os/src/fs/fd.rs`, `os/src/syscall/fs.rs`, `os/src/task/wait_queue.rs` | Regular file read/write readiness is immediate, including EOF. Pipe read readiness is based on buffered bytes or writer shutdown; pipe write readiness is based on reader presence and remaining pipe capacity. `POLLHUP` and write-side `POLLERR` are reported from pipe endpoint state. `ppoll`/`pselect6` timeouts use validated signed `timespec` values and wait on IO queues without busy looping. Socket readiness is `missing` because socket syscalls are not dispatched. |
...
| Futex: `futex WAIT/WAKE` | `partial` | `os/src/syscall/other.rs`, `os/src/task/wait_queue.rs` | `WAIT` validates aligned user addresses, compares the user word before sleeping, rechecks before blocking, supports relative timeout, and returns `EAGAIN`/`ETIMEDOUT`/`EINTR` on the real wait path. `WAKE` validates the futex address and wakes address/key-matched waiters. Unsupported futex ops return `ENOSYS` instead of fake success. PI, requeue, robust list, bitset operations, and shared-process details are `missing`. |
