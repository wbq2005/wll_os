...
LMBENCH ?= 1
LTP ?= 0
# This submission branch is the CAgent deliverable.  Keep workload selection
# explicit at build time so the evaluator's plain `make all` produces kernels
# that launch the CAgent script from the attached official ext4 image.
HARNESS_GROUPS ?= cagent
